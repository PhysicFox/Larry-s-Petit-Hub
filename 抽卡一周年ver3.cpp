...
char up6[10][10] = { "W","温蒂" };
char up5[10][10] = { "极境" }; //行数控制最大值 赋给个数 列数解决上次每一个概率要手打的问题
int main() {
	system("color F0");
	int c6 = 0, c5 = 0, c4 = 0, c3 = 0, u6 = 2, u5 = 1;
	for (int i = 0; i < 100; i++) {
		if (ss[i][0]) c6++;
		if (aa[i][0]) c5++;
		if (bb[i][0]) c4++;
...
HEAD:
	...
	cout << "-------------------------拉里秘制明日方舟抽卡模拟器\t一周年限定--------------------------" << endl << "是否开启干员获得率Up？输入1开启 输入0关闭" << endl;
	int open;
...
while (1) {
	...
	fout << "感谢使用！祝发发出货！" << endl;
	fout << "by LarryLiu" << endl;
	fout.close();
