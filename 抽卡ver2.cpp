...
using namespace std;
int a,b,c,d,e; 
char ss[100][10]= {"刻俄柏","年","阿","煌","麦哲伦","塞雷娅","莫斯提马","能天使","推进之王","伊芙利特","艾雅法拉","闪灵","安洁莉娜","夜莺","星熊","银灰","斯卡蒂","陈","黑","赫拉格"};
char aa[100][10]= {"惊蛰","吽","雪雉","灰喉","布洛卡","苇草","拜松","槐琥","炎客","送葬人","白面鸮","凛冬","德克萨斯","芙兰卡","拉普兰德","幽灵鲨","蓝毒","白金","陨星","天火","梅尔","赫默","华法琳","临光","红","雷蛇","可颂","普罗旺斯","守林人","崖心","初雪","真理","空","狮蝎","食铁兽","夜魔","诗怀雅","格劳克斯","星极"};
char bb[100][10]= {"安比尔","清流","夜烟","远山","杰西卡","流星","白雪","清道夫","红豆","杜宾","缠丸","霜叶","慕斯","砾","暗索","末药","调香师","角峰","蛇屠箱","古米","深海色","地灵","阿消","猎蜂","格雷伊","苏苏洛","桃金娘","红云","梅"};
char cc[100][10]= {"芬","香草","翎羽","玫兰莎","卡缇","米格鲁","克洛丝","炎熔","芙蓉","安赛尔","史都华德","梓兰","空爆","月见夜","斑点","泡普卡"};
char up6[10][10]= {a,b};
char up5[10][10]= {c,d,e}; //行数控制最大值 赋给个数 列数解决上次每一个概率要手打的问题
int main() {
	system("color F0");
...
HEAD:
    ...
    cout<<"看看自己花了多长时间在 抽 六 星上（迫真)"<<endl;
    cout << "-------------------------拉里秘制明日方舟抽卡模拟器--------------------------" << endl << "是否开启干员获得率Up？输入1开启 输入0关闭" << endl;
	int open;
	cin >> open;//用char写Y/N在第二个分支跳进for里确实很麻烦 解决办法有什么头猪吗？ 
LOOP:	if(open==1) {
		cout<<"请输入获得率提升的六星干员（*2）" <<endl;
		for(int i=0; i<2; i++){
			cin>>up6[i]; 
		}
		cout<<"请输入获得率提升的五星干员（*3）"<<endl;
		for(int i=0; i<3; i++){
			cin>>up5[i];
		}
		cout << "以下干员获得率提升！！" << endl ;
		...
		for(int i=0; i<chou; i++) {
			...
			if(a>=six) {
				...
				if(open) {
					...
					// 注：这里的概率其实并不准确，因为歪的干员中也存在Up的干员，就会导致Up的概率比实际要高  这里还没想出解决的方法(不过准一点不好吗/滑稽   
			...
			if(baodi>=50) {
				...
				// 比如第一次降低后六星爆率为4% 五星爆率就是(100%-4%)/98*8 可能这么算有问题 欢迎指正 
				six-=200; // 六星-200即为-2% 
				five=six-six/98*8;
				four=five-six/98*50;
			}
